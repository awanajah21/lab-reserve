document.addEventListener("DOMContentLoaded", () => {
      ...
      loginForm.addEventListener("submit", (e) => {
        ...
        if (userExists) {
            if (userExists.password === password) {
                ...
                if (userExists.role === 'admin') navigateAndClearHistory("Admin.html");
                else navigateAndClearHistory("home.html");
            } else {
                alert("❌ Login Gagal: Password yang Anda masukkan salah. Periksa kembali.");
            }
        } else {
  ...
  // =========================================================
    ...
    if (registerForm) {
      registerForm.addEventListener("submit", (e) => {
        ...
        alert(`🎉 Registrasi berhasil! Akun '${username}' berhasil dibuat. Silakan Login.`);
        navigateAndClearHistory("index.html"); 
      });
    }
  }
  ...
  // =========================================================
    ...
    if (form) {
      ...
      if (submitButton) { submitButton.setAttribute('type', 'submit'); submitButton.removeAttribute('onclick'); }
  ...
  // =========================================================
    ...
    const backToDashboardButton = document.querySelector('button[onclick*="RoomList.html"]');
    const viewScheduleButton = document.querySelector('button[onclick*="Jadwal.html"]');
    if (backToDashboardButton) backToDashboardButton.onclick = () => navigateTo("RoomList.html");
    if (viewScheduleButton) viewScheduleButton.onclick = () => navigateTo("Jadwal.html");

    const reservations = JSON.parse(localStorage.getItem("reservasiData"));
    if (reservations && reservations.length > 0) {
        const lastReservation = reservations[reservations.length - 1];
        
        document.querySelector(".space-y-4 div:nth-child(1) p:last-child").textContent = lastReservation.lab || "N/A";
        document.querySelector(".space-y-4 div:nth-child(2) p:last-child").textContent = lastReservation.date || "N/A";
        document.querySelector(".space-y-4 div:nth-child(3) p:last-child").textContent = lastReservation.time || "N/A";
        const purposeElement = document.querySelector(".space-y-4 div:nth-child(5) p:last-child");
        if(purposeElement) purposeElement.textContent = lastReservation.purpose || "N/A";
    }
  }
  ...
  // =========================================================
    ...
    const reservationsTableBody = document.getElementById("reservation-table-body");
    const searchReservationsInput = document.getElementById("search-input");
    const adminLogoutButton = document.getElementById("admin-logout-btn"); // Ambil ID tombol Logout

    // Penanganan Logout di Admin.html
    ...
    const renderReservations = (reservationsToRender) => {
        ...
        if (!reservationsToRender || reservationsToRender.length === 0) {
            reservationsTableBody.innerHTML = '<tr><td colspan="6" class="text-center py-4 text-gray-500 dark:text-gray-400">Tidak ada reservasi yang ditemukan.</td></tr>';
            return;
        }
    ...
    window.deleteReservation = (index) => {
        if (confirm("⚠️ Apakah Anda yakin ingin menghapus reservasi ini?")) {
            let reservations = JSON.parse(localStorage.getItem("reservasiData")) || [];
    ...
    if (searchReservationsInput) {
        searchReservationsInput.addEventListener("keyup", (e) => {
            ...
            const allReservations = JSON.parse(localStorage.getItem("reservasiData")) || [];
            const filteredReservations = allReservations.filter((res) => {
                return Object.values(res).some((value) => typeof value === "string" && value.toLowerCase().includes(searchTerm));
            });
  ...
  // =========================================================
  // 6. LOGIKA HALAMAN LAINNYA (RoomList.html)
  // =========================================================
...
function initProfileFeatures(currentUser, navigateTo) {
    ...
    const loadProfilePhoto = () => {
        const photoUrl = localStorage.getItem("userProfilePhoto_" + currentUser.username);
    ...
    if (profileButton && profileMenu) {
        // Karena Admin.html sudah ditangani secara terpisah, ini khusus untuk Mahasiswa
        const roleDisplay = currentUser.role.charAt(0).toUpperCase() + currentUser.role.slice(1);
        
        if(dropdownUsernameElement) dropdownUsernameElement.textContent = currentUser.username;
        if(dropdownRoleElement) dropdownRoleElement.textContent = roleDisplay;
        if(profileInitials) profileInitials.textContent = currentUser.username.charAt(0).toUpperCase();
        ...
        // FUNGSI LOGOUT (Hanya untuk halaman Mahasiswa/umum)
        if (logoutBtn) {
            logoutBtn.addEventListener("click", () => {
                if (confirm("Anda yakin ingin keluar (Logout)?")) {
                    localStorage.removeItem("currentUser");
                    window.location.replace("index.html"); // Menggunakan replace untuk logout yang bersih
                }
            });
        }
        ...
        document.addEventListener("click", (e) => {
            ...
            if (container && !container.contains(e.target) && !profileMenu.classList.contains("hidden")) {
    ...
    if (modal) {
        ...
        if(changePhotoBtn) {
            changePhotoBtn.addEventListener("click", () => {
                if(profileMenu) profileMenu.classList.add("hidden", "scale-95", "opacity-0");
                if(profileMenu) profileMenu.classList.remove("scale-100", "opacity-100");
        ...
        if(cancelUploadBtn) {
             cancelUploadBtn.addEventListener("click", () => {
                ...
                if(selectPhotoBtn) selectPhotoBtn.textContent = 'Pilih File Gambar (.jpg, .png)';
            });
        }
        ...
        if(photoInput) {
            photoInput.addEventListener("change", (e) => {
                ...
                if (file) {
                    uploadedFile = file;
                    if(selectPhotoBtn) selectPhotoBtn.textContent = `File dipilih: ${file.name}`;
        ...
        if(form) {
            form.addEventListener("submit", (e) => {
                ...
                reader.onload = () => {
                    ...
                    localStorage.setItem("userProfilePhoto_" + currentUser.username, base64Image);
